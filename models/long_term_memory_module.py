...
class LongTermMemoryModule(nn.Module):
    ...
    def _initialize_memory_bank(self, memory_init_embedding, phase_type,
        ...
        if memory_init_embedding is None:
            if phase_type == 'test_long':
                # Test phase: load saved memory
                if dataset_name == 'UCR_Anomaly':
                    file_name_without_ext = os.path.splitext(file_name)[0]
                    load_path = f'save_models/VLoLo/{run_name}/{run_id}/memory_item/{dataset_name}/{file_name_without_ext}/{dataset_name}_long_term_memory_item.pth'
                else:
                    load_path = f'save_models/VLoLo/{run_name}/{run_id}/memory_item/{dataset_name}_long_term_memory_item.pth'
                ...
                print(f'{load_path} - loading long-term memory banks for {self.n_vars} variables (test phase)')
                self.register_buffer('mem_bank', init_mem)
            else:
                # First training phase: random initialization
                print(f'Initializing long-term memory banks randomly for {self.n_vars} variables (first train phase)')
    ...
    def forward(self, query):
        ...
        """
        is_training = (self.phase_type != 'test_long')
